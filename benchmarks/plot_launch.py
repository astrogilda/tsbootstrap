...
the four overlapping methods (8 cores, n=2000, B=10000); right, peak memory
before and after on the two headline reduce workloads, on a linear scale so the
reduction is shown at true magnitude.
...
def main() -> None:
    ...
    for i, s in enumerate(speed):
        ax_l.text(
            s + 0.06, i, f"{s:g}x", va="center", ha="left", color=FG, fontsize=14, fontweight="bold"
        )
    ...
    # Right: peak memory before/after, linear scale.
    work = ["VAR residual\n(n=1000, d=3, B=20k)", "Panel reduce\n(1000 series, B=1000)"]
    before = [480.0, 1600.0]
    after = [1.4, 8.0]
    red = ["343x less", "200x less"]
    lab_before = ["480 MB", "1.6 GB"]
    lab_after = ["1.4 MB", "8 MB"]
    x = range(len(work))
    w = 0.36
    ax_r.bar(
        [i - w / 2 for i in x],
    ...
    fig.suptitle(
        "tsbootstrap 0.3.0: faster than arch, a fraction of the memory",
        fontsize=18,
        fontweight="bold",
        color=FG,
        x=0.5,
