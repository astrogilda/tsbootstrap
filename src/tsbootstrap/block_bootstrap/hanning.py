"""
Hanning Block Bootstrap implementation.
"""
...
class HanningBootstrap(BaseBlockBootstrap):
    r"""
    Hanning Bootstrap class for time series data with Hanning window tapering.

    This class applies Hanning window tapering to blocks during resampling.
    ...
    tapered_weights : callable, default=np.hanning
        Fixed to np.hanning for Hanning window tapering.
    ...
    bootstrap_type: Literal["moving"] = Field(
        default="moving", description="Always 'moving' for Hanning Bootstrap."
    )
    tapered_weights: Optional[Callable] = Field(
        default=None,
        description="Always np.hanning for Hanning window tapering.",
    )
    ...
    def __init__(self, **data: Any) -> None:
        ...
        if "tapered_weights" in data and (
            data["tapered_weights"] == "hanning" or isinstance(data["tapered_weights"], str)
        ):
            data["tapered_weights"] = np.hanning
        super().__init__(**data)

    def model_post_init(self, __context: ConfigDict) -> None:
        """Set tapered_weights to np.hanning after initialization."""
        # Use object.__setattr__ to bypass validation
        if self.tapered_weights is None:
            object.__setattr__(self, "tapered_weights", np.hanning)
    ...
    def get_params(self, deep: bool = True) -> dict[str, Any]:
        ...
        if "tapered_weights" in params and (
            ...
            params["tapered_weights"] = "hanning"
        return params

    def set_params(self, **params: Any) -> "HanningBootstrap":
        """Set parameters for this estimator."""
        # Convert string back to function if needed
        if "tapered_weights" in params and (
            params["tapered_weights"] == "hanning" or isinstance(params["tapered_weights"], str)
        ):
            params["tapered_weights"] = np.hanning
