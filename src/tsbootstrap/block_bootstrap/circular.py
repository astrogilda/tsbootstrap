"""
Circular Block Bootstrap implementation.
"""
...
class CircularBlockBootstrap(BlockBootstrap):
    ...
    * `wrap_around_flag` is always set to True, meaning that the data will
    ...
    * `combine_generation_and_sampling_flag` is always False, meaning that the block
    ...
    .. math::
        \\hat{X}_t = \\frac{1}{L}\\sum_{i=1}^L X_{(t + \\lfloor U_i \\rfloor) \\mod n}

    where :math:`L` is the block length, :math:`U_i` is a uniform random variable on :math:`[0, 1]`, :math:`\\lfloor \\cdot \\rfloor` is the floor function, and :math:`n` is the number of observations.
