"""
Non-Overlapping Block Bootstrap implementation.
"""
...
class NonOverlappingBlockBootstrap(BlockBootstrap):
    ...
    * `overlap_flag` is always set to False, meaning that blocks do not overlap.
    ...
    * `combine_generation_and_sampling_flag` is always False, meaning that the block
    ...
    .. math::
        \\hat{X}_t = \\frac{1}{L}\\sum_{i=1}^L X_{kL + i}

    where :math:`L` is the block length, :math:`k` is the block index, and :math:`n` is the number of observations.
