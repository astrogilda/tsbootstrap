"""
Bartlett's Block Bootstrap implementation.
"""
...
class BartlettsBootstrap(BaseBlockBootstrap):
    r"""
    Bartlett's Bootstrap class for time series data with Bartlett window tapering.

    This class applies Bartlett window tapering to blocks during resampling.
    ...
    tapered_weights : callable, default=np.bartlett
        Fixed to np.bartlett for Bartlett window tapering.
    ...
    .. math::
        ...
        2 - \frac{2n}{N-1} & \frac{N-1}{2} < n \leq N-1
        \end{cases}
    ...
    bootstrap_type: Literal["moving"] = Field(
        default="moving",
        description="Always 'moving' for Bartlett's Bootstrap.",
    )
    tapered_weights: Optional[Callable] = Field(
        default=None,
        description="Always np.bartlett for Bartlett window tapering.",
    )
    ...
    def __init__(self, **data: Any) -> None:
        ...
        if "tapered_weights" in data and (
            data["tapered_weights"] == "bartlett" or isinstance(data["tapered_weights"], str)
        ):
            data["tapered_weights"] = np.bartlett
        super().__init__(**data)

    def model_post_init(self, __context: ConfigDict) -> None:
        """Set tapered_weights to np.bartlett after initialization."""
        # Use object.__setattr__ to bypass validation
        if self.tapered_weights is None:
            object.__setattr__(self, "tapered_weights", np.bartlett)
    ...
    def get_params(self, deep: bool = True) -> dict[str, Any]:
        ...
        if "tapered_weights" in params and (
            ...
            params["tapered_weights"] = "bartlett"
        return params

    def set_params(self, **params: Any) -> "BartlettsBootstrap":
        """Set parameters for this estimator."""
        # Convert string back to function if needed
        if "tapered_weights" in params and (
            params["tapered_weights"] == "bartlett" or isinstance(params["tapered_weights"], str)
        ):
            params["tapered_weights"] = np.bartlett
