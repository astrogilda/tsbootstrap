...
class TestBlockBootstrapServices:
    ...
    def test_block_generation_length_validation(self):
        ...
        with pytest.raises(
            ValueError, match="block_length cannot be greater than the size of the input array"
        ):
    ...
    def test_block_generation_service_comprehensive_parameters(self):
        ...
        blocks = service.generate_blocks(X, block_length=8, block_length_distribution="exponential")
        assert len(blocks) > 0

        # Test with min_block_length parameter
        blocks = service.generate_blocks(X, block_length=10, min_block_length=3)
        assert len(blocks) > 0
