...
class TestSkbaseCompatibility:
    def test_get_params_set_params_clone_roundtrip(self):
    ...
    def test_adapters_are_discovered_by_skbase(self):
    ...
    def test_sktime_check_estimator_runs_clean(self):
...
class TestBlockAdapters:
    ...
    def test_block_adapter_yields_samples(self, cls):
        x = ar1(0.5, 100, 0)
    ...
    def test_return_indices(self):
        x = ar1(0.5, 60, 1)
        out = list(
            StationaryBlockBootstrap(avg_block_length=5, n_bootstraps=4, random_state=0).bootstrap(
    ...
    def test_determinism_through_adapter(self):
        x = ar1(0.5, 80, 3)
...
class TestIIDAdapter:
    def test_iid_adapter(self):
        x = ar1(0.0, 80, 1)
...
class TestModelAdapters:
    def test_model_adapters_yield_samples(self):
        x = ar1(0.6, 200, 2)
        for est in (
            ARResidualBootstrap(order=1, n_bootstraps=5, random_state=0),
            SieveBootstrap(n_bootstraps=5, random_state=0),
        ):
            samples = list(est.bootstrap(x))
    ...
    def test_var_adapter_multivariate(self):
