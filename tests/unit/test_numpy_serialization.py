...
class TestNumpySerializationService:
    ...
    def test_serialize_dicts(self):
        ...
        input_dict = {
            ...
            "scalar": np.int64(42),
            "nested": {"inner_array": np.array([4, 5]), "string": "test"},
        }
        ...
        expected = {
            "array": [1, 2, 3],
            "scalar": 42,
            "nested": {"inner_array": [4, 5], "string": "test"},
        }
    ...
    def test_serialize_pydantic_models(self):
        ...
        model_data = {"array": np.array([1, 2, 3]), "scalar": np.float64(3.14), "string": "test"}
        mock_model = MockPydanticModel(model_data)

        result = service.serialize_numpy_arrays(mock_model)
        expected = {"array": [1, 2, 3], "scalar": 3.14, "string": "test"}
        assert result == expected
    ...
    def test_validate_array_input_non_array_permissive(self):
        ...
        with pytest.raises(
            TypeError, match="cannot be converted to a numpy array even in permissive mode"
        ):
            service.validate_array_input(NonConvertible())
    ...
    def test_comprehensive_edge_cases(self):
        ...
        complex_data = {
            ...
            "other": {"string": "test", "int": 42, "rng": np.random.default_rng(42), "none": None},
        }
