...
class TestTaperedBlock:
    ...
    def test_runs_and_shape(self, name):
        x = ar1(0.6, 200, 0)
        res = bootstrap(
            x, method=TaperedBlock(window=name, block_length=10), n_bootstraps=5, random_state=1
        )
    ...
    def test_determinism(self):
        x = ar1(0.5, 150, 2)
        a = bootstrap(
            x, method=TaperedBlock(window="hann", block_length=8), n_bootstraps=6, random_state=3
        )
        b = bootstrap(
            x, method=TaperedBlock(window="hann", block_length=8), n_bootstraps=6, random_state=3
        )
    ...
    def test_variance_preservation_gate(self, name):
        x = ar1(0.5, 400, 4)
        res = bootstrap(
            x, method=TaperedBlock(window=name, block_length=20), n_bootstraps=400, random_state=5
        )
    ...
    def test_mean_preservation(self):
        x = ar1(0.6, 400, 6)
        res = bootstrap(
            x, method=TaperedBlock(window="hann", block_length=20), n_bootstraps=400, random_state=7
        )
