...
class TestBlockGenerator:
    ...
    def test_non_overlapping_blocks(self):
        ...
        assert len(unique_indices) == len(
            all_indices
        ), "Found overlapping indices in non-overlapping blocks"
    ...
    def test_circular_blocks(self):
        ...
        generator = BlockGenerator(
            input_length=20, block_length_sampler=sampler, wrap_around_flag=True
        )
        blocks = generator.generate_blocks(overlap_flag=False)
    ...
    def test_variable_length_blocks(self):
        ...
        sampler = BlockLengthSampler(avg_block_length=6, block_length_distribution="geometric")
        generator = BlockGenerator(
            input_length=30,
...
class TestBlockLengthSampler:
    ...
    def test_geometric_distribution(self):
        ...
        sampler = BlockLengthSampler(
            avg_block_length=20, block_length_distribution="geometric", rng=rng
        )
    ...
    def test_uniform_distribution(self):
        ...
        rng = np.random.default_rng(42)
        sampler = BlockLengthSampler(
            avg_block_length=15, block_length_distribution="uniform", rng=rng
        )

        lengths = [sampler.sample_block_length() for _ in range(1000)]

        assert all(
            1 <= length < 30 for length in lengths
        )  # uniform samples 1 to 2*avg_block_length
        assert 14 <= np.mean(lengths) <= 16  # Should be around 15

    def test_fixed_length(self):
        """Test fixed block length (no distribution)."""
        sampler = BlockLengthSampler(avg_block_length=25, block_length_distribution=None)
