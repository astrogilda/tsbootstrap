...
class TestBootstrap:
    class TestErrorConditions:
        ...
        @pytest.mark.parametrize("data, block_length, bootstrap_class", [(1, 1, cls) for cls in bootstrap_classes])
        def test_invalid_data_type(self, data, block_length, bootstrap_class):
            with pytest.raises(TypeError):
                bootstrap_class(data).split(block_length)

        @pytest.mark.parametrize("block_length, bootstrap_class", [(i, cls) for i in [-1, 0] for cls in bootstrap_classes])
        def test_non_positive_block_length(self, block_length, bootstrap_class):
    ...
    class TestNormalConditions:
        ...
        @given(st.lists(st.floats(allow_nan=False), min_size=1), st.integers(min_value=1))
        ...
        @given(st.lists(st.floats(allow_nan=True, allow_infinity=True), min_size=2), st.integers(min_value=1))
        @settings(deadline=None)
        @pytest.mark.parametrize("bootstrap_class", bootstrap_classes)
        def test_nan_values_counts(self, data, block_length, bootstrap_class):
            tsb = bootstrap_class(data)

            original_nan_or_inf = [str(x) if np.isnan(
                x) or np.isinf(x) else x for x in data]
            ...
            for _ in range(bootstrap_iterations):
                for train_index, _ in tsb.split(block_length):
                    bootstrap_nan_or_inf = [str(x) if np.isnan(
                        x) or np.isinf(x) else x for x in train_index]
