...
integer_array = st.lists(st.integers(
    min_value=1, max_value=10), min_size=10, max_size=10).map(np.array)
float_array = st.lists(st.floats(
    allow_nan=False, allow_infinity=False, min_value=-1e10, max_value=1e10), min_size=10, max_size=10).map(lambda x: np.array(x).reshape(-1, 1))
float_array_unique = st.just(np.random.rand(10, 1))


def ar_model_strategy():
    return st.builds(lambda data: AutoReg(data, lags=1).fit(), float_array_unique)


def arima_model_strategy():
    return st.builds(lambda data: ARIMA(data, order=(1, 0, 0)).fit(), float_array_unique)


def sarima_model_strategy():
    return st.builds(lambda data: SARIMAX(data, order=(1, 0, 0), seasonal_order=(0, 0, 0, 0)).fit(), float_array_unique)


def var_model_strategy():
    return st.builds(lambda data: VAR(data).fit(maxlags=1), float_array_unique.map(lambda x: np.column_stack([x, x])))
